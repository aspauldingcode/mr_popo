...
if (!TOKEN || !WATCHED_USER_ID || !PROTECTED_USER_ID) {
  console.error(
    'Missing required environment variables (DISCORD_TOKEN, WATCHED_USER_ID, PROTECTED_USER_ID)'
  );
  process.exit(1);
}
    ...
    try {
        ...
        if (member) {
          if (member.bannable) {
            ...
            await message.channel.send(
              `Failed to ban <@${author.id}>. Please check my role permissions.`
            );
          }
        }
      }
      ...
      if (LOG_CHANNEL_ID) {
        ...
        if (logChannel && logChannel.isTextBased()) {
          const actionText =
            offenseCount >= MAX_OFFENSES
              ? '**BANNED**'
              : `**WARNING ${offenseCount}/${MAX_OFFENSES}**`;
