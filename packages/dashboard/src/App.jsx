...
const Dashboard = () => {
    ...
    const handleReset = (userId) => {
        fetch(`/api/offenses/reset/${userId}`, { method: 'POST' })
            .then(res => res.json())
            .then(data => {
                if (data.success) fetchOffenses();
            });
    };

    const handleResetAll = () => {
        if (window.confirm('Are you sure you want to reset ALL offenses?')) {
            fetch('/api/offenses/reset-all', { method: 'POST' })
                .then(res => res.json())
                .then(data => {
                    if (data.success) fetchOffenses();
                });
                ...
                <main className="grid gap-8">
                    <div className="flex justify-end">
                        <button
                            ...
                            className="bg-red-500/10 text-red-400 px-4 py-2 rounded-lg hover:bg-red-500/20 transition border border-red-500/20"
                        >
                            Reset All Offenses
                        </button>
                    </div>
                            ...
                            <tbody>
                                {offenses.length > 0 ? offenses.map(offense => (
                                    <tr key={offense.user_id} className="border-b border-white/5 last:border-0">
                                        ...
                                        <td className="p-4">
                                            <button
                                                onClick={() => handleReset(offense.user_id)}
